...
- (void)viewDidLoad {
    ...
    //设置导航栏标题
    [self.navigationItem setTitle:@"服药明细"];
    //获取状态栏的宽高
...
-(void)initSegmentedControlView{
    ...
    NSDictionary *attributes = [NSDictionary dictionaryWithObjectsAndKeys:[UIFont boldSystemFontOfSize:12],NSFontAttributeName,[UIColor segmentedBlueColor], NSForegroundColorAttributeName, nil, nil];
...
- (PageViewController *)viewControllerAtIndex:(NSUInteger)index {
    ...
    // 根据页面名称创建页面对象
    if(index==0){
        PageViewController *viewController = [[MedicationDetailPage1 alloc] initWithNibName:[pageContent objectAtIndex:index] bundle:nil];
        viewController.paramDic = _paramDic;
        viewController.dataObject =[pageContent objectAtIndex:index];
        return viewController;
    }else{
        PageViewController *viewController = [[MedicationDetailPage2 alloc] initWithNibName:[pageContent objectAtIndex:index] bundle:nil];
        viewController.paramDic = _paramDic;
        viewController.dataObject =[pageContent objectAtIndex:index];
        return viewController;
    }
