...
- (void)medicate_now:(UIButton *)sender{
    ...
    for(NSDictionary *tempDic in _array){
        ...
        if([res isEqualToString:@"true"]){
            ...
            //发送刷新界面的广播
            [[NSNotificationCenter defaultCenter] postNotificationName:@"medication" object:nil];
            //设置药品剩余剂量减1，返回药品剩余药量
    ...
    //注销加时闹钟
    [NotificationUtil cancelExtraNotification:_alarm_time];
}

//如果点击了稍后服药
- (void)medicate_later:(UIButton *)sender{
    //添加加时闹钟
    [NotificationUtil setExtraNotificationWithTimeStr:_alarm_time];
    //关闭当前界面
    if(_isRootView){
        [self exitAppliction];
    }else{
