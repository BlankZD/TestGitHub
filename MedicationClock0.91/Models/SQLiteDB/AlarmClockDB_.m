...
+(NSMutableArray*)query:(BaseDB*)dbDriver byAlarmTime:(NSString*)alarmTime{
    ...
    sqlite3 *database = [dbDriver getDatabase];
    NSMutableArray *res = [[NSMutableArray alloc]init];
    NSString *selectSQL = [NSString stringWithFormat:@"select %@,%@,%@,%@,%@ from %@ inner join %@ on %@._id=%@.%@  and %@.%@='1' where %@.%@='%@' ", CLOCK_ID, CLOCK_TITLE, CLOCK_CONTENT, EXPIRE_DOSE, TIME_STR, TimeTableName, ClockTableName, ClockTableName, TimeTableName, CLOCK_ID, ClockTableName, STATE, TimeTableName, TIME_STR, alarmTime];
    sqlite3_stmt *statement;    //这个相当于ODBC的Command对象，用于保存编译好的SQL语句
    if (sqlite3_prepare(database, [selectSQL UTF8String], -1, &statement, nil) == SQLITE_OK){
        //Sqlite3_step(), 在调用sqlite3_prepare后，使用这个函数在记录集中移动。
        while (sqlite3_step(statement) == SQLITE_ROW){
