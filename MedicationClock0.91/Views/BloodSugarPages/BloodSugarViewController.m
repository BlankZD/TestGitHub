...
-(void)initView{
    ...
    [_dateLabelView setTextColor:[UIColor whiteColor]];
     _dateLabelView .font=[UIFont systemFontOfSize:13*SizeScale];
...
-(UITableViewCell *)customCellByXib:(UITableView *)tableView withIndexPath:(NSIndexPath *)indexPath{
     ...
     */
    //fyx 20160607 修改tabeview中信息显示不全问题
    NSString *blood_sugar = [dic objectForKey:@"blood_sugar"];
    UILabel *label2 = ((UILabel *)[cell.contentView viewWithTag:2]);
   // [label2 setTextColor:[UIColor whiteColor]];
    if([AppConfig isGlucopenia:blood_sugar]){
        [label2 setTextColor:[UIColor whiteColor]];
    }else{
        [label2 setTextColor:[UIColor redColor]];
    }
    label2.text = [[NSString alloc] initWithFormat:@"血糖：%@", blood_sugar];
    label2.font=[UIFont systemFontOfSize:15];
