...
-(void)drawRectDateNumber:(CGRect)rect context:(CGContextRef)context{
    ...
    if ([[self getComponentsForDate:_secDate] month] == [[self getComponentsForDate:_currDate] month] && [[self getComponentsForDate:_secDate] year] == [[self getComponentsForDate:_currDate] year]) {
        //算出选中日期所在的格子位置
    ...
    for (int i=0; i<NumBlocks; i++) {
        ...
        //绘制日期数字
        [dateNumber drawInRect:CGRectMake(targetX, targetY+10, Calendar_DayWidth, Calendar_DayHeight) withFont:[UIFont boldSystemFontOfSize:Calendar_DayWidth/3.5] lineBreakMode:NSLineBreakByClipping alignment:NSTextAlignmentCenter];
    }
